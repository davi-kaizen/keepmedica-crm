...
# CORS — permite o frontend Next.js (porta 3000) acessar a API
...
login_manager.login_view = 'api_auth_login'
...
def init_db():
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS pipelines (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL)''')
    cursor.execute('''
    ...
    # --- MIGRAÇÕES AUTOMÁTICAS (Para suportar DB existente) ---
    try:
        # Verifica e adiciona colunas novas em appointments se não existirem
        ...
        # A coluna color já existia no código anterior, mas garantimos
        if 'color' not in appt_cols:
            cursor.execute("ALTER TABLE appointments ADD COLUMN color TEXT DEFAULT '#3b82f6'")
            
        # Verifica visible em doctors
        ...
        # Verifica colunas antigas de Leads/Users
        existing_lead_cols = [c[1] for c in cursor.execute("PRAGMA table_info(leads)")]
        if 'profile_pic' not in existing_lead_cols: cursor.execute("ALTER TABLE leads ADD COLUMN profile_pic TEXT")
        
        existing_user_cols = [c[1] for c in cursor.execute("PRAGMA table_info(users)")]
        if 'meta_token' not in existing_user_cols: cursor.execute("ALTER TABLE users ADD COLUMN meta_token TEXT")
        if 'ig_page_id' not in existing_user_cols: cursor.execute("ALTER TABLE users ADD COLUMN ig_page_id TEXT")
            
    except Exception as e:
        print(f"[DB WARN] Erro na verificação de migração: {e}")

    # Inicializa Pipelines Padrão
    ...
    # Inicializa Médicos Padrão (Se vazio)
...
def log_action(user_id, description, pipeline_id, details=""):
    try:
        conn = get_db()
        conn.execute("INSERT INTO activities (user_id, description, details, pipeline_id) VALUES (?, ?, ?, ?)", 
...
def format_relative_time(dt_str):
    ...
    except: return "Recente"

# ==============================================================================
# FUNÇÃO WORKER CORRIGIDA (SEM 'PLATFORM')
# ==============================================================================
def worker_fetch_leads(user_id, meta_token, ig_page_id):
    ...
    try:
        ...
        # CORREÇÃO: Removemos o parâmetro 'platform': 'instagram' que causava retorno vazio
        params = {'fields': 'id,updated_time,messages{message,from,created_time},participants', 'access_token': meta_token, 'limit': 20}
        
        resp = requests.get(url, params=params)
        data = resp.json()
        
        # Debug no console para ajudar a ver o que chega
        print(f"DEBUG FETCH: {data}")
        ...
        for conv in conversations:
            ...
            results.append({"name": client.get('name', username), "username": username, "profile_pic": "", "last_msg": msg_text, "time_ago": format_relative_time(conv.get('updated_time')), "id": conv.get('id')})
...
<head>
    ...
    <style type="text/tailwindcss">
        body { font-family: 'Inter', system-ui, sans-serif; @apply bg-blue-50/30 dark:bg-slate-900 dark:text-slate-100; }
        ...
        .msg-me { @apply bg-[#d9fdd3] dark:bg-green-800 dark:text-white; align-self: flex-end; border-top-right-radius: 0; }
        ...
        .nav-btn-custom.active-leads { @apply bg-indigo-100 border-indigo-300 text-indigo-700 dark:bg-indigo-900/50 dark:border-indigo-700 dark:text-indigo-300 shadow-inner ring-1 ring-indigo-200 dark:ring-indigo-800; }
        .nav-btn-custom.leads-hover:hover { @apply text-indigo-600 border-indigo-200 bg-indigo-50/50 dark:hover:bg-indigo-900/30; }
        .nav-btn-custom.active-chat { @apply bg-green-100 border-green-300 text-green-700 dark:bg-green-900/50 dark:border-green-700 dark:text-green-300 shadow-inner ring-1 ring-green-200 dark:ring-green-800; }
        .nav-btn-custom.chat-hover:hover { @apply text-green-600 border-green-200 bg-green-50/50 dark:hover:bg-green-900/30; }
        .nav-btn-custom.active-agenda { @apply bg-cyan-100 border-cyan-300 text-cyan-700 dark:bg-cyan-900/50 dark:border-cyan-700 dark:text-cyan-300 shadow-inner ring-1 ring-cyan-200 dark:ring-cyan-800; }
        .nav-btn-custom.agenda-hover:hover { @apply text-cyan-600 border-cyan-200 bg-cyan-50/50 dark:hover:bg-cyan-900/30; }


        .nav-scroll-btn {
            position: fixed; top: 55%; transform: translateY(-50%); width: 50px; height: 50px;
            background: rgba(59, 130, 246, 0.85); color: white; border-radius: 50%;
        ...
        .nav-scroll-btn:hover { background: #2563eb; transform: translateY(-50%) scale(1.1); }
        #btn-left { left: 20px; }
        ...
        .menu-item { @apply flex items-center justify-start gap-4 px-6 py-3 text-sm text-gray-700 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-slate-700 hover:text-blue-600 transition-all duration-200 cursor-pointer mx-2 rounded-xl; }
        ...
        .tab-btn.active { @apply bg-blue-50 dark:bg-slate-700 text-blue-600 dark:text-blue-400 font-bold; }
        ...
        .card-menu-item:hover { color: #2563eb; }
        ...
        .card-red { border-top: 4px solid #f87171; }
        .card-red .card-link { color: #ef4444; }
        ...
        .card-pink { border-top: 4px solid #f472b6; }
        .card-pink .card-link { color: #ec4899; }
        .card-pink:hover { border-color: #ec4899; }

        .card-purple { border-top: 4px solid #a78bfa; }
        ...
        .card-blue { border-top: 4px solid #60a5fa; }
        ...
        .mini-cal-day.active { @apply bg-blue-600 text-white font-bold; }

    </style>
</head>
<body class="h-screen flex flex-col overflow-hidden text-gray-800 dark:text-slate-100 bg-[#f8faff] dark:bg-slate-900">

    {% if not current_user.is_authenticated %}
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-100 dark:bg-slate-900">
        <div class="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl w-96 text-center border border-gray-200 dark:border-slate-700">
            <div class="w-16 h-16 bg-blue-600 rounded-xl mx-auto flex items-center justify-center text-white text-3xl font-bold mb-6 shadow-lg shadow-blue-200 dark:shadow-none">K</div>
            <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">KeepMed</h2>
            
            <form action="/login" method="POST" class="space-y-4">
                <input name="username" class="w-full p-3 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" placeholder="Usuário" required>
                <input name="password" type="password" class="w-full p-3 border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" placeholder="Senha" required>
                <button type="submit" class="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg transition shadow-md">Entrar</button>
            </form>
    ...
    <div id="modal-admin-panel" class="fixed inset-0 z-[70] bg-gray-100 dark:bg-slate-900 hidden flex items-center justify-center">
        <div class="bg-white dark:bg-slate-800 w-[1000px] h-[750px] rounded-xl shadow-2xl flex flex-col overflow-hidden animate-fade-in-up">
            ...
            <div class="flex flex-1 overflow-hidden">
                <aside class="w-64 bg-gray-50 dark:bg-slate-900 border-r border-gray-200 dark:border-slate-700 p-4 space-y-2">
                    <button onclick="setTab('users')" class="w-full text-left p-3 rounded-lg hover:bg-white dark:hover:bg-slate-800 hover:shadow transition font-medium text-gray-700 dark:text-slate-300 flex items-center gap-2"><i class="fas fa-users text-blue-500"></i> Usuários</button>
                ...
                <main class="flex-1 p-8 overflow-y-auto bg-white dark:bg-slate-800">
                    <div id="tab-users">
                        ...
                        <div class="bg-blue-50 dark:bg-slate-700 p-5 rounded-xl border border-blue-100 dark:border-slate-600 mb-8">
                            <h4 class="text-sm font-bold text-blue-800 dark:text-blue-300 mb-3 uppercase tracking-wide">Criar Novo Acesso</h4>
                            <div class="flex gap-3">
                                <input id="new-u" placeholder="Nome de Usuário" class="border border-blue-200 dark:border-slate-500 dark:bg-slate-800 dark:text-white p-2.5 rounded-lg text-sm flex-1 focus:ring-2 focus:ring-blue-400 outline-none">
                                <input id="new-p" placeholder="Senha" class="border border-blue-200 dark:border-slate-500 dark:bg-slate-800 dark:text-white p-2.5 rounded-lg text-sm flex-1 focus:ring-2 focus:ring-blue-400 outline-none">
                                <button onclick="admCreateUser()" class="bg-blue-600 hover:bg-blue-700 text-white px-6 rounded-lg text-sm font-bold shadow-md transition">CRIAR</button>
                            </div>
                        </div>
    ...
    <header class="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 h-16 flex items-center justify-between px-6 z-20 shrink-0 shadow-sm relative">
        <div class="flex items-center gap-8 h-full">
            <div class="flex items-center gap-3 cursor-pointer hover:opacity-80 transition group relative" onclick="viewApp('hub')">
                <div class="w-9 h-9 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-lg flex items-center justify-center text-white font-bold text-xl shadow transform group-active:scale-95 transition-transform"><i class="fas fa-heartbeat text-sm"></i></div>
                <div class="leading-tight">
                    <h1 class="font-bold text-gray-800 dark:text-white text-lg">KeepMed</h1>
        ...
        <div class="flex items-center gap-6">
            ...
            <div class="relative">
                <button onclick="toggleNotifications()" class="text-gray-500 dark:text-slate-400 hover:text-blue-600 transition relative">
                ...
                <div id="notif-portal">
                    <div class="bg-white dark:bg-slate-800 h-full flex flex-col">
                        <div class="p-4 border-b dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-900">
                            ...
                            <button onclick="clearAllNotifs()" class="text-xs text-blue-500 hover:underline">Limpar todas</button>
                        </div>
            ...
            <div class="relative">
                ...
                <div id="profile-menu">
                    <div class="menu-header">
                        ...
                        <div class="w-16 h-16 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white font-bold text-2xl shadow-lg mb-3 border-4 border-white dark:border-slate-700">
                    ...
                    <div class="py-1">
                        <div class="menu-item group" onclick="openProfileModal()">
                            <i class="far fa-user w-5 group-hover:scale-110 group-hover:rotate-6 transition-transform text-blue-600"></i> 
                            Configurações da Conta
                            <span class="ml-auto opacity-0 group-hover:opacity-100 text-blue-500 text-[10px] font-bold transition-opacity">VER</span>
                        </div>
    ...
    <div class="flex-1 overflow-hidden relative">
        ...
        <div id="view-hub" class="absolute inset-0 overflow-y-auto p-6 transition-opacity duration-300">
            <div class="max-w-full mx-auto px-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
                    ...
                    <div onclick="viewApp('kanban')" class="new-hub-card card-orange group">
                        <div>
                            ...
                            <p class="card-desc">Gerencie seus pacientes potenciais através de um pipeline visual. Acompanhe cada etapa da negociação via chat.</p>
                        </div>
                    ...
                    <div onclick="viewApp('chat')" class="new-hub-card card-green group">
                        <div>
                            ...
                            <p class="card-desc">Centralize WhatsApp e Instagram. Responda rápido e converta conversas em vendas.</p>
                        </div>
                        <span class="card-link">Acessar <i class="fas fa-chevron-right text-xs"></i></span>
                    </div>
                    <div onclick="viewApp('agenda')" class="new-hub-card card-cyan group">
                        <div>
                            ...
                            <p class="card-desc">Visualização completa da agenda médica. Organize consultas, retornos e procedimentos de forma integrada.</p>
                        </div>
                        <span class="card-link">Acessar <i class="fas fa-chevron-right text-xs"></i></span>
                    </div>
                    ...
                    <div onclick="viewApp('quality')" class="new-hub-card card-lime group">
                        <div>
                            ...
                            <p class="card-desc">Análise detalhada de performance. Monitore tempos de resposta, taxas de conversão e eficiência da equipe.</p>
                        </div>
                        <span class="card-link">Acessar <i class="fas fa-chevron-right text-xs"></i></span>
                    </div>
                    ...
                    <div onclick="openProfileModal()" class="new-hub-card card-blue group">
                        <div>
                            <div class="card-top-icon bg-blue-100 text-blue-500 dark:bg-blue-900/30">
                                <i class="fas fa-user-shield"></i>
                            </div>
                            <h3 class="card-title">Usuários</h3>
                            <p class="card-desc">Controle de acesso e permissões. Gerencie médicos, recepcionistas e administradores do sistema.</p>
                        </div>
                        <span class="card-link">Acessar <i class="fas fa-chevron-right text-xs"></i></span>
                    </div>
                ...
                <div class="text-center mt-12 text-sm text-gray-400">
                    <i class="far fa-lightbulb text-yellow-500 mr-2"></i> Dica: O sucesso do atendimento começa na rapidez da resposta.
                </div>
            </div>
        </div>
            ...
            <div class="w-80 border-r border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex flex-col p-6">
                <div class="mb-6 flex justify-between items-center">
                    ...
                    <div class="flex gap-2">
                        <button onclick="openDoctorsModal()" class="text-gray-400 hover:text-cyan-600 mr-2" title="Gerenciar Médicos"><i class="fas fa-cog"></i></button>
                        <button onclick="changeMonth(-1)" class="text-gray-400 hover:text-blue-600"><i class="fas fa-chevron-left"></i></button>
                        <button onclick="changeMonth(1)" class="text-gray-400 hover:text-blue-600"><i class="fas fa-chevron-right"></i></button>
                    </div>
                </div>
        ...
        <div id="view-kanban" class="absolute inset-0 overflow-x-auto p-6 flex gap-5 items-start transition-opacity duration-300 hidden opacity-0">
            ...
            <div class="flex-shrink-0 w-80 flex flex-col bg-gray-50/80 dark:bg-slate-800/80 rounded-xl border border-gray-200 dark:border-slate-700 max-h-full shadow-sm">
                ...
                <div class="p-3 flex-1 overflow-y-auto kanban-col scrollbar-hide space-y-3" 
                    ...
                    <div draggable="true" ondragstart="drag(event, '{{ lead['id'] }}')" 
                         class="bg-white dark:bg-slate-700 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-slate-600 cursor-grab active:cursor-grabbing hover:shadow-md hover:border-blue-300 dark:hover:border-blue-500 transition-all group relative">
                        <div class="absolute top-3 right-3 z-20">
                            ...
                            <div id="card-menu-{{ lead['id'] }}" class="card-menu">
                                <div onclick="openEditLead({{ lead['id'] }}, '{{ lead['name'] }}', '{{ lead['value'] }}')" class="card-menu-item">
                                    <i class="fas fa-pencil-alt text-blue-500 w-4"></i> Editar
                                </div>
                        ...
                        <div class="flex items-center gap-3 mb-3 pr-6">
                            ...
                            <div class="overflow-hidden">
                                ...
                                <a href="https://instagram.com/{{ lead['username'] }}" target="_blank" class="text-xs text-blue-500 hover:underline flex items-center gap-1">
            ...
            <div onclick="addKanbanColumn()" class="golden-btn group">
                <div class="w-16 h-16 rounded-full bg-yellow-400 dark:bg-yellow-600 flex items-center justify-center text-white text-3xl shadow-lg group-hover:scale-110 transition-transform">
                    <i class="fas fa-plus"></i>
                </div>
                <span class="mt-4 font-bold text-yellow-600 dark:text-yellow-400 uppercase tracking-widest text-sm">Adicionar Coluna</span>
            </div>
        </div>
            ...
            <div class="w-1/3 max-w-sm border-r border-gray-200 dark:border-slate-700 flex flex-col bg-white dark:bg-slate-800 z-10 shadow-lg">
                <div class="p-5 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <button onclick="loadThreads()" class="w-8 h-8 rounded-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-300 hover:text-blue-600 hover:border-blue-300 flex items-center justify-center transition shadow-sm"><i class="fas fa-sync-alt"></i></button>
                    </div>
                    <div class="relative">
                        <select id="chat-filter" onchange="applyFilter()" class="w-full p-2.5 pl-3 border border-gray-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 outline-none appearance-none cursor-pointer font-medium text-gray-700 shadow-sm">
                ...
                <div id="chat-input" class="p-4 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700 hidden">
                    <form onsubmit="sendMsg(event)" class="flex gap-3 items-end max-w-4xl mx-auto">
                        <div class="flex-1 relative">
                            <textarea id="msg-txt" rows="1" class="w-full p-3 pl-4 border border-gray-300 dark:border-slate-600 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none resize-none bg-gray-50 dark:bg-slate-700 text-gray-700 dark:text-white shadow-inner" placeholder="Digite sua mensagem..." onkeydown="if(event.keyCode == 13 && !event.shiftKey) { event.preventDefault(); sendMsg(event); }"></textarea>
                        </div>
                        <button type="submit" class="bg-blue-600 text-white w-12 h-12 rounded-full hover:bg-blue-700 transition shadow-lg flex items-center justify-center transform hover:scale-105 active:scale-95"><i class="fas fa-paper-plane"></i></button>
                    </form>
                </div>
            </div>
        </div>

        <div id="view-quality" class="absolute inset-0 overflow-y-auto p-8 hidden bg-gray-100 dark:bg-slate-900">
            <h2 class="text-3xl font-bold mb-2 text-gray-800 dark:text-white">Dashboard de Qualidade</h2>
            <p class="text-gray-500 dark:text-slate-400 mb-8">Visão geral do seu atendimento clínico</p>
            <div class="grid grid-cols-4 gap-6 mb-8">
                <div class="dash-card" style="background-color: #4A6FA5;"><i class="far fa-comments text-white"></i><h4 class="text-sm font-medium opacity-80 text-white">Leads Abordados</h4><p class="text-4xl font-bold mt-2 text-white">0</p></div>
                <div class="dash-card" style="background-color: #C45B5B;"><i class="fas fa-hourglass-half text-white"></i><h4 class="text-sm font-medium opacity-80 text-white">Leads em follow up</h4><p class="text-4xl font-bold mt-2 text-white">0</p></div>
                <div class="dash-card" style="background-color: #5C946E;"><i class="far fa-calendar-check text-white"></i><h4 class="text-sm font-medium opacity-80 text-white">Leads Agendamento</h4><p class="text-4xl font-bold mt-2 text-white">0</p></div>
                <div class="dash-card" style="background-color: #D49E4C;"><i class="fas fa-chart-line text-white"></i><h4 class="text-sm font-medium opacity-80 text-white">Leads em desenvolvimento</h4><p class="text-4xl font-bold mt-2 text-white">0</p></div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-96 overflow-hidden animate-fade-in-up">
            ...
            <div class="p-6 space-y-3">
                ...
                <div>
                    ...
                    <input id="appt-procedure" list="proc-list" class="w-full border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded p-2 text-sm focus:ring-2 focus:ring-cyan-500 outline-none" placeholder="Ex: Consulta Rotina">
                ...
                <div class="flex gap-3">
                    ...
                    <div>
                        ...
                        <input type="color" id="appt-color" value="#3b82f6" class="h-9 w-12 border-none rounded cursor-pointer bg-transparent">
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-96 overflow-hidden animate-fade-in-up transform scale-100 transition-transform">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input id="edit-appt-patient" class="w-full border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                
                <div>
                    <label class="block text-xs font-bold text-gray-500 dark:text-slate-400 mb-1">Procedimento</label>
                    <input id="edit-appt-procedure" list="proc-list" class="w-full border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                </div>

                <div class="flex gap-3">
                    <div class="flex-1">
                        <label class="block text-xs font-bold text-gray-500 dark:text-slate-400 mb-1">Duração (min)</label>
                        <input type="number" id="edit-appt-duration" min="5" step="5" class="w-full border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 dark:text-slate-400 mb-1">Cor</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 dark:text-slate-400 mb-1">Nota/Observação</label>
                    <input id="edit-appt-note" class="w-full border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                ...
                <div class="flex gap-2 pt-2">
                    ...
                    <button onclick="saveApptEdit()" class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded shadow-md transition text-xs">SALVAR</button>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-[500px] overflow-hidden animate-fade-in-up">
            ...
            <div class="p-6">
                <div class="flex gap-2 mb-6">
                    ...
                    <button onclick="addDoctor()" class="bg-blue-600 hover:bg-blue-700 text-white px-4 rounded font-bold text-sm">ADICIONAR</button>
                </div>
                
                <h4 class="text-xs font-bold text-gray-500 dark:text-slate-400 uppercase mb-3">Lista Atual</h4>
                <div id="doctors-list-settings" class="space-y-2 max-h-60 overflow-y-auto pr-2">
                    </div>
            </div>
        </div>
    </div>
    ...
    <div id="modal-instagram" class="fixed inset-0 z-[90] bg-black/60 hidden flex items-center justify-center backdrop-blur-sm transition-opacity">
        <div class="bg-white dark:bg-slate-800 w-[400px] rounded border border-gray-300 dark:border-slate-700 flex flex-col items-center p-8 relative animate-fade-in-up shadow-2xl">
            ...
            <div id="insta-form" class="w-full flex flex-col gap-3">
                <textarea id="insta-token" class="w-full border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded p-2 text-xs h-24 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Cole seu Token (EAA...) aqui..."></textarea>
                <button id="btn-insta-login" onclick="connectInstagram()" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded text-sm transition">CONECTAR</button>
                <p id="insta-error" class="text-xs text-red-500 text-center hidden"></p>
            </div>
            <div id="insta-loading" class="hidden flex flex-col items-center py-4">
                <div class="loader mb-3"></div>
                <p class="text-xs text-gray-500 dark:text-slate-400 font-medium">Validando Token...</p>
            </div>
        </div>
    </div>
    ...
    <div id="modal-import" class="fixed inset-0 z-[90] bg-black/70 hidden flex items-center justify-center backdrop-blur-sm">
         <div class="bg-white dark:bg-slate-800 w-[900px] h-[600px] rounded-xl flex flex-col overflow-hidden relative">
            ...
            <div class="p-6 border-b border-gray-200 dark:border-slate-700 flex justify-between items-center bg-gray-50 dark:bg-slate-900">
                ...
                <button onclick="runImport()" class="bg-blue-600 text-white px-4 py-2 rounded text-sm font-bold shadow hover:bg-blue-700 transition"><i class="fas fa-sync-alt animate-spin-slow"></i> Iniciar Varredura</button>
            </div>
            <div class="flex-1 bg-gray-100 dark:bg-slate-900 p-6 overflow-y-auto relative">
                <div id="import-empty" class="flex flex-col items-center justify-center h-full text-gray-400">
                    <i class="fas fa-inbox text-6xl mb-4 opacity-20"></i>
                    <p>Clique em "Iniciar Varredura" para buscar novas conversas.</p>
                </div>
                <div id="import-loading" class="absolute inset-0 bg-white/80 dark:bg-slate-800/80 z-20 flex flex-col items-center justify-center hidden">
                    <div class="loader mb-4 w-12 h-12 border-4"></div>
                    <h3 class="text-lg font-bold text-gray-700 dark:text-white">Analisando Directs...</h3>
                </div>
            ...
            <div class="flex-1 relative bg-white dark:bg-slate-800">
                ...
                <div id="tab-profile" class="tab-content active h-full overflow-y-auto">
                    <div class="h-32 bg-gray-200 dark:bg-slate-700 w-full relative mb-12">
                        <div class="absolute -bottom-10 left-8 w-24 h-24 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 border-4 border-white dark:border-slate-800 flex items-center justify-center text-white text-3xl font-bold shadow-lg">
                            {{ current_user.username[0]|upper }}
                        </div>
                    </div>
                    <div class="px-8 pb-8">
                        <div class="mb-6"><h2 class="text-xl font-bold text-gray-800 dark:text-white">{{ current_user.username }}</h2><p class="text-sm text-gray-500 dark:text-slate-400">@{{ current_user.username }}</p></div>
                    </div>
                </div>
                <div id="tab-activity" class="tab-content h-full overflow-y-auto">
                    <div class="p-8"><h2 class="text-lg font-bold text-gray-800 dark:text-white mb-6 flex items-center gap-2"><i class="fas fa-list"></i> Atividade Recente</h2><div id="profile-activity-list" class="space-y-0"><div class="loader"></div></div></div>
                </div>
                <div id="tab-cards" class="tab-content h-full flex flex-col items-center justify-center text-gray-400"><i class="far fa-id-card text-5xl mb-4 opacity-50"></i><p>Você não tem cartões.</p></div>
                <div id="tab-settings" class="tab-content h-full flex flex-col items-center justify-center text-gray-400"><i class="fas fa-cogs text-5xl mb-4 opacity-50"></i><p>Configurações.</p></div>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-96 overflow-hidden animate-fade-in-up transform scale-100 transition-transform">
            ...
            <div class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <input id="edit-lead-name" class="w-full border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-500 dark:text-slate-400 mb-1">Valor da Negociação (R$)</label>
                    <input id="edit-lead-value" type="number" step="0.01" class="w-full border border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded p-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none">
                </div>
                <button onclick="saveLeadEdit()" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 rounded shadow-md transition mt-2">SALVAR ALTERAÇÕES</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Carrega larguras salvas ou inicia vazio
        ...
        document.addEventListener('keyup', e => { delete keys[e.key.toLowerCase()]; if(timer){clearTimeout(timer); timer=null;} });
        ...
        function openInstaModal() { document.getElementById('modal-instagram').classList.remove('hidden'); }
        function closeInstaModal() { document.getElementById('modal-instagram').classList.add('hidden'); }
        ...
        function connectInstagram() {
            ...
            if(!token) { errorMsg.innerText = "Cole o token."; errorMsg.classList.remove('hidden'); return; }
            form.classList.add('hidden'); loading.classList.remove('hidden'); errorMsg.classList.add('hidden');
            fetch('/api/instagram/connect', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({token: token}) })
            .then(async r => { if (!r.ok) { const text = await r.text(); throw new Error(`Erro do Servidor (${r.status}): Verifique o terminal Python.`); } return r.json(); })
            .then(data => { if(data.success) { closeInstaModal(); document.getElementById('modal-import').classList.remove('hidden'); alert("Instagram Conectado! ID: " + data.page_id); } else { throw new Error(data.error || "Erro desconhecido na API."); } })
            .catch(err => { form.classList.remove('hidden'); loading.classList.add('hidden'); errorMsg.innerText = err.message; errorMsg.classList.remove('hidden'); console.error("Debug Erro:", err); });
        }
        ...
        function setTab(t) {
            ...
            if(t==='pipelines') loadAdminPipes(); else loadAdminUsers();
        }
        ...
        function loadAdminUsers() {
            fetch('/api/admin/users').then(r=>r.json()).then(d => {
                ...
                d.users.forEach(u => {
                    l.innerHTML += `<div class="bg-white dark:bg-slate-700 p-4 rounded-xl border border-gray-200 dark:border-slate-600 flex items-center justify-between shadow-sm hover:shadow-md transition"><div><div class="font-bold text-gray-800 dark:text-white text-lg">${u.username}</div><div class="text-xs text-gray-400 font-mono">ID: ${u.id}</div></div><div class="flex items-center gap-3"><div class="flex flex-col"><label class="text-[10px] font-bold text-gray-400 uppercase">Session ID</label><div class="flex gap-1"><input id="s-${u.id}" value="${u.session_id||''}" class="border rounded text-xs p-1.5 w-40 bg-gray-50 dark:bg-slate-800 dark:border-slate-600 dark:text-white focus:bg-white dark:focus:bg-slate-600 transition" placeholder="Cole aqui..."><button onclick="saveSess(${u.id})" class="text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 px-2 rounded"><i class="fas fa-save"></i></button></div></div><div class="flex flex-col"><label class="text-[10px] font-bold text-gray-400 uppercase">Pipeline #</label><div class="flex gap-1"><input id="p-${u.id}" value="${u.pipeline_id}" type="number" class="border rounded text-xs p-1.5 w-16 text-center bg-gray-50 dark:bg-slate-800 dark:border-slate-600 dark:text-white focus:bg-white dark:focus:bg-slate-600 transition"><button onclick="savePipe(${u.id})" class="text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 px-2 rounded"><i class="fas fa-check"></i></button></div></div><button onclick="delUser(${u.id})" class="text-red-400 hover:text-red-600 ml-2 p-2 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-full transition"><i class="fas fa-trash"></i></button></div></div>`;
                });
            });
        }
        function admCreateUser() { fetch('/api/admin/users', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:document.getElementById('new-u').value,password:document.getElementById('new-p').value})}).then(()=>loadAdminUsers()); }
        function saveSess(id) { fetch('/api/admin/users/update', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id,session_id:document.getElementById(`s-${id}`).value})}).then(()=>alert('Session Salvo!')); }
        function savePipe(id) { fetch('/api/admin/users/update', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id,pipeline_id:document.getElementById(`p-${id}`).value})}).then(()=>alert('Pipeline Vinculado!')); }
        function delUser(id) { if(confirm('Tem certeza?')) fetch('/api/admin/users/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})}).then(()=>loadAdminUsers()); }
        function loadAdminPipes() {
            fetch('/api/admin/pipelines').then(r=>r.json()).then(d => {
                const l = document.getElementById('list-pipelines'); l.innerHTML = '';
                d.pipelines.forEach(p => {
                    let stHtml = '';
                    p.stages.forEach(s => { stHtml += `<div class="flex justify-between items-center text-xs bg-gray-50 dark:bg-slate-900 p-2 rounded mb-1 border border-gray-100 dark:border-slate-600 group"><span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full shadow-sm" style="background:${s.color}"></span><span class="font-medium text-gray-700 dark:text-slate-300">${s.name}</span></span><button onclick="delStage(${s.id})" class="text-red-300 hover:text-red-600 opacity-0 group-hover:opacity-100 transition">&times;</button></div>`; });
                    l.innerHTML += `<div class="bg-white dark:bg-slate-800 p-5 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm hover:shadow-md transition"><div class="flex justify-between items-center border-b dark:border-slate-600 pb-3 mb-3"><h4 class="font-bold text-lg text-gray-800 dark:text-white">#${p.id} - ${p.name}</h4><button onclick="delPipe(${p.id})" class="text-red-500 text-xs font-bold hover:bg-red-50 dark:hover:bg-red-900/30 px-2 py-1 rounded transition">EXCLUIR CONJUNTO</button></div><div class="mb-4 pl-2 border-l-4 border-gray-100 dark:border-slate-700 space-y-1">${stHtml}</div><div class="flex gap-2 bg-gray-50 dark:bg-slate-900 p-2 rounded-lg border border-gray-200 dark:border-slate-700"><input id="st-n-${p.id}" placeholder="Nome da Nova Coluna" class="border-none bg-transparent text-xs flex-1 outline-none dark:text-white"><input id="st-c-${p.id}" type="color" value="#3b82f6" class="w-6 h-6 rounded cursor-pointer border-none"><button onclick="addStage(${p.id})" class="bg-blue-600 text-white px-3 py-1 rounded text-xs font-bold hover:bg-blue-700 transition">ADD</button></div></div>`;
                });
            });
        }
        function admCreatePipe() { fetch('/api/admin/pipelines',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:document.getElementById('new-pipe').value})}).then(()=>loadAdminPipes()); }
        function delPipe(id) { if(confirm('ATENÇÃO: Apagará todos os leads e colunas deste funil!')) fetch('/api/admin/pipelines/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})}).then(()=>loadAdminPipes()); }
        function addStage(pid) { fetch('/api/admin/stages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pipeline_id:pid,name:document.getElementById(`st-n-${pid}`).value,color:document.getElementById(`st-c-${pid}`).value})}).then(()=>loadAdminPipes()); }
        function delStage(sid) { if(confirm('Remover coluna?')) fetch('/api/admin/stages/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:sid})}).then(()=>loadAdminPipes()); }
        ...
        function viewApp(v) {
            const views = ['hub', 'kanban', 'chat', 'quality', 'agenda'];
            views.forEach(view => {
                const el = document.getElementById(`view-${view}`);
                if (el) { el.classList.add('hidden'); if (view !== 'hub' && view !== 'quality') el.classList.add('opacity-0'); }
                const btn = document.getElementById(`nav-${view}`);
                if (btn) { btn.classList.remove('active-leads', 'active-chat', 'active-agenda'); btn.classList.add('inactive'); }
            });

            const selected = document.getElementById(`view-${v}`);
            if (selected) { selected.classList.remove('hidden'); if (v !== 'hub' && v !== 'quality') setTimeout(() => selected.classList.remove('opacity-0'), 10); }

            const activeBtn = document.getElementById(`nav-${v}`);
            if (activeBtn) {
                activeBtn.classList.remove('inactive');
                if (v === 'kanban' || v === 'hub') activeBtn.classList.add('active-leads');
                if (v === 'chat') activeBtn.classList.add('active-chat');
                if (v === 'agenda') { 
                    activeBtn.classList.add('active-agenda'); 
                    // Carrega os médicos antes de renderizar
        ...
        function closeDoctorsModal() {
            ...
            loadDoctorsAndRender(); // Refresh agenda view
        }
        ...
        function loadDoctorsSettings() {
            ...
            .then(data => {
                ...
                data.doctors.forEach(doc => {
                    // Define o ícone com base na visibilidade
                    let eyeIcon = doc.visible ? 'fa-eye text-blue-500' : 'fa-eye-slash text-gray-400';
                    ...
                    <div class="flex items-center gap-2 bg-gray-50 dark:bg-slate-700 p-2 rounded border dark:border-slate-600">
                        <button onclick="toggleDoctorVisibility(${doc.id}, ${doc.visible})" class="hover:bg-gray-200 dark:hover:bg-slate-600 p-1.5 rounded transition" title="${titleText}">
                        ...
                        <button onclick="updateDoctor(${doc.id})" class="text-blue-500 hover:text-blue-700 text-xs font-bold px-2">SALVAR</button>
        ...
        // --- AGENDA LOGIC (COM REDIMENSIONAMENTO & VISIBILIDADE) ---
        function renderAgenda() {
            ...
            // Filtrar apenas médicos visíveis
            const visibleDoctors = DOCTORS.filter(d => d.visible);

            // Render Doctors Header
            visibleDoctors.forEach(doc => {
                const cleanName = doc.name.replace(/\s/g, '');
                let style = colWidths[cleanName] 
                    ? `width: ${colWidths[cleanName]}px; flex: none;` 
                    : `flex: 1; min-width: 150px;`;

                header.innerHTML += `
                    <div class="doctor-header relative" id="header-${cleanName}" style="${style}">
                        ${doc.name}
                        <div class="resizer" onmousedown="startResize(event, '${cleanName}')"></div>
                    </div>`;
            });

            // Render Columns for each Doctor
            visibleDoctors.forEach(doc => {
                const cleanName = doc.name.replace(/\s/g, '');
                let style = colWidths[cleanName] 
                    ? `width: ${colWidths[cleanName]}px; flex: none;` 
            ...
            fetch(`/api/appointments?date=${dateStr}`).then(r=>r.json()).then(data => {
                data.appointments.forEach(appt => {
                    const docIdClean = appt.doctor.replace(/\s/g,'');
                    const slotId = `slot-${docIdClean}-${appt.time}`;
                    const slot = document.getElementById(slotId);
                    if(slot) {
                        // VISUALIZAÇÃO: Calcula a altura baseada na duração
                        // 15 min = 1 célula = 40px (do CSS .calendar-cell {h-10})
                        // Formula: (duracao / 15) * 40px - 2px (margem)
        ...
        // NOVA FUNÇÃO: Abrir Modal de Edição (Com novos campos)
        ...
        function deleteAppointment() {
            ...
            if(confirm("Deseja realmente desmarcar esta consulta?")) {
                fetch('/api/appointments/delete', {
                    ...
                    if(data.success) {
                        document.getElementById('modal-edit-appointment').classList.add('hidden');
                        renderAgenda(); // Recarrega a agenda
                    } else {
        ...
        function renderMiniCalendar() {
            ...
            for(let d=1; d<=lastDay.getDate(); d++) {
                let isToday = (d === new Date().getDate() && currentDate.getMonth() === new Date().getMonth() && currentDate.getFullYear() === new Date().getFullYear());
        ...
        function openApptModal(doc, time) {
            ...
            document.getElementById('appt-color').value = '#3b82f6';
        ...
        function runImport(){
            ...
            fetch('/api/start_fetch').then(r=>r.json()).then(d => {
                if(d.error) {
                    alert(d.error);
                    document.getElementById('import-loading').classList.add('hidden');
                    if(d.error.includes("Instagram não conectado") || d.error.includes("Token")) openInstaModal();
                } else {
        ...
        function checkImport(){
            fetch('/api/get_candidates').then(r=>r.json()).then(d=>{
                if(!d.loading){
        ...
        function renderImport(list){
            const g = document.getElementById('import-grid'); g.innerHTML='';
            if(!list || list.length===0){document.getElementById('import-empty').classList.remove('hidden');return;}
            list.forEach((u,idx)=>{
                let btns=''; 
                MY_STAGES.forEach(s=>{
                ...
                g.innerHTML+=`<div class="h-72 perspective"><div class="flip-inner shadow-lg rounded-xl bg-white dark:bg-slate-800 h-full" id="card-${idx}"><div class="flip-front p-6 border border-gray-200 dark:border-slate-600 flex flex-col items-center justify-between bg-white dark:bg-slate-800 h-full"><div class="flex flex-col items-center w-full"><div class="w-20 h-20 rounded-full border-4 border-white dark:border-slate-700 shadow-md mb-3 bg-gray-200 dark:bg-slate-600 flex items-center justify-center text-2xl font-bold text-gray-400 dark:text-slate-300">${u.name[0]}</div><h3 class="font-bold text-gray-800 dark:text-white text-sm truncate w-full text-center">${u.name}</h3><p class="text-xs text-blue-500 mb-2">@${u.username}</p><span class="text-[10px] bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-0.5 rounded-full font-bold mb-3">${u.time_ago}</span><div class="w-full bg-gray-50 dark:bg-slate-900 p-2 rounded text-[10px] text-gray-500 dark:text-slate-400 italic text-center line-clamp-2 h-10 border dark:border-slate-700">"${u.last_msg}"</div></div><button onclick="document.getElementById('card-${idx}').classList.add('flipped')" class="w-full bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold py-2.5 rounded-lg transition shadow-md mt-2">ADICIONAR</button></div><div class="flip-back p-4 overflow-y-auto bg-white dark:bg-slate-800 h-full flex flex-col"><div class="flex justify-between items-center mb-3 pb-2 border-b dark:border-slate-600"><span class="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">Destino</span><button onclick="document.getElementById('card-${idx}').classList.remove('flipped')" class="text-red-400 hover:text-red-600 font-bold text-xs">VOLTAR</button></div><div class="grid grid-cols-2 gap-2 content-start overflow-y-auto flex-1">${btns}</div></div></div></div>`;
            });
        }
        function addToCrm(u,s){
            fetch('/api/get_candidates').then(r=>r.json()).then(d=>{
                ...
                if(c){
                    c.target_stage=s;
                    fetch('/api/confirm_lead',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(c)}).then(r=>r.json()).then(res=>{if(res.success)location.reload();});
                }
            });
        }
        ...
        function loadThreads(){
            ...
            list.innerHTML='<div class="flex flex-col items-center justify-center h-48 space-y-3"><div class="loader"></div><p class="text-gray-400 text-sm">Sincronizando com Instagram...</p></div>';
            fetch('/api/chat/threads').then(r=>r.json()).then(d=>{
                if(!d.success){ list.innerHTML=`<div class="p-6 text-center"><p class="text-red-500 font-bold mb-2">Erro de Conexão</p><p class="text-xs text-gray-500">${d.error}</p><button onclick="loadThreads()" class="mt-4 text-blue-600 text-sm underline">Tentar Novamente</button></div>`; return; }
                GLOBAL_THREADS = d.threads; applyFilter();
            }).catch(err => { list.innerHTML=`<div class="p-6 text-center text-red-500 text-sm">Erro de rede. Verifique o servidor.</div>`; });
        }
        ...
        function applyFilter() {
            ...
            if(filtered.length === 0) { list.innerHTML = '<div class="p-10 text-center text-gray-400 text-sm italic">Nenhuma conversa com este filtro.</div>'; return; }
            filtered.forEach(t => {
                ...
                list.innerHTML += `<div onclick="openChat('${t.thread_id}','${t.user.name}','${t.user.username}','${t.user.pic}', '${t.lead_info ? t.lead_info.status : ''}', '${t.lead_info ? t.lead_info.color : ''}')" class="p-4 border-b border-gray-100 dark:border-slate-700 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer flex items-center gap-3 transition group bg-white dark:bg-slate-800"><div class="w-12 h-12 rounded-full bg-gray-200 dark:bg-slate-600 flex items-center justify-center font-bold text-gray-500 dark:text-slate-300 shrink-0">${t.user.name[0]}</div><div class="overflow-hidden flex-1"><div class="flex items-center justify-between w-full mb-1"><h4 class="font-bold text-sm text-gray-800 dark:text-white truncate group-hover:text-blue-700 dark:group-hover:text-blue-400 transition">${t.user.name}</h4>${badge}</div><p class="text-xs text-gray-500 dark:text-slate-400 truncate group-hover:text-gray-700 dark:group-hover:text-slate-200">${t.last_msg}</p></div></div>`;
            });
        }
        ...
        function openChat(tid, name, user, pic, status, color){
            ...
            if(status) { badge.classList.remove('hidden'); badge.innerText = status; badge.style.backgroundColor = color; } else { badge.classList.add('hidden'); }
            ...
            fetch(`/api/chat/messages?thread_id=${tid}`).then(r=>r.json()).then(d=>{
                area.innerHTML = '';
                if(d.success) { d.messages.reverse().forEach(m => { area.innerHTML += `<div class="msg-bubble ${m.is_sent_by_me ? 'msg-me' : 'msg-them'}">${m.text}</div>`; }); area.scrollTop = area.scrollHeight; } else { area.innerHTML = '<div class="text-center text-red-400 mt-10">Erro ao carregar mensagens.</div>'; }
            });
        }
        ...
        function sendMsg(e){
            ...
            if(!txt||!tid) return;
            const area = document.getElementById('chat-msgs'); 
            area.innerHTML += `<div class="msg-bubble msg-me opacity-70">${txt}</div>`; area.scrollTop = area.scrollHeight; 
            document.getElementById('msg-txt').value='';
            fetch('/api/chat/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({thread_id:tid,text:txt})});
        }

        function allowDrop(e){e.preventDefault();e.currentTarget.classList.add('drag-over');}
        function drop(e,st){e.preventDefault();e.currentTarget.classList.remove('drag-over');fetch('/update_stage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:e.dataTransfer.getData("text"),status:st})}).then(()=>location.reload());}
        function drag(e,id){e.dataTransfer.setData("text",id);}
        function deleteLead(id){if(confirm("Excluir definitivamente?"))fetch('/delete_lead',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id})}).then(()=>location.reload());}
        function scrollKanban(amount) { const container = document.getElementById('view-kanban'); if(container) { container.scrollBy({ left: amount, behavior: 'smooth' }); } }

        function toggleNotifications() {
            const portal = document.getElementById('notif-portal');
            const menu = document.getElementById('profile-menu');
            const mainMenu = document.getElementById('main-menu');
            menu.style.display = 'none'; if(mainMenu) mainMenu.style.display = 'none';
        ...
        function loadNotifications() {
            fetch('/api/notifications').then(r=>r.json()).then(data => {
                ...
                if(data.total > 0) { badge.classList.remove('hidden'); badge.innerText = data.total >= 10 ? '9+' : data.total; } else { badge.classList.add('hidden'); }
                list.innerHTML = '';
                if(data.notifs.length === 0) { list.innerHTML = '<p class="text-center text-gray-400 text-sm py-10">Nenhuma notificação nova.</p>'; } else {
                    data.notifs.forEach(n => { list.innerHTML += `<div class="p-3 border-b hover:bg-blue-50 dark:hover:bg-slate-700 transition flex items-center gap-3 relative"><img src="${n.profile_pic}" class="w-10 h-10 rounded-full border"><div class="flex-1 overflow-hidden"><p class="text-xs font-bold text-gray-800 dark:text-white">${n.name}</p><p class="text-[11px] text-gray-500 dark:text-slate-400 truncate italic">"${n.last_msg}"</p></div><div class="flex flex-col items-center gap-1"><span class="bg-blue-100 text-blue-600 text-[9px] font-bold px-1.5 rounded-full">${n.unread_count}</span><button onclick="clearNotif(${n.id})" class="text-gray-300 hover:text-blue-500 transition"><i class="fas fa-check-circle"></i></button></div></div>`; });
                }
            });
        }
        function clearNotif(id) { fetch('/api/notifications/clear', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({id:id})}).then(()=>loadNotifications()); }
        function clearAllNotifs() { fetch('/api/notifications/clear', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({})}).then(()=>loadNotifications()); }

        function toggleProfileMenu() {
            const menu = document.getElementById('profile-menu');
            const portal = document.getElementById('notif-portal');
            const mainMenu = document.getElementById('main-menu');
            portal.style.display = 'none'; if(mainMenu) mainMenu.style.display = 'none';
        ...
        function toggleMainMenu() {
            ...
            const portal = document.getElementById('notif-portal');
            portal.style.display = 'none'; profileMenu.style.display = 'none';
        ...
        function loadActivities() {
            ...
            fetch('/api/activities').then(r=>r.json()).then(data => {
                let html = '';
                if(data.activities.length === 0) { html = '<div class="flex flex-col items-center justify-center h-full text-gray-400"><i class="fas fa-history text-4xl mb-2 opacity-50"></i><p class="text-sm">Sem atividades recentes.</p></div>'; } else {
                    data.activities.forEach(act => { html += `<div class="activity-item relative flex gap-4 pb-6"><div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-md shrink-0 z-10 border-2 border-white dark:border-slate-800">${act.user_initial}</div><div class="flex-1"><p class="text-sm text-gray-800 dark:text-white"><span class="font-bold">${act.user_name}</span> ${act.description}</p><span class="text-[11px] text-gray-400 dark:text-slate-500 flex items-center gap-1 mt-1"><i class="far fa-clock"></i> ${act.time_ago}</span></div></div>`; });
                }
...
# --- API AUTH (JSON - usado pelo frontend Next.js) ---
...
def api_auth_login():
    ...
    u = cur.fetchone()
    conn.close()
    if u and (check_password_hash(u['password'], password) or u['password'] == password):
        login_user(User(u['id'], u['username'], u['password'], u['meta_token'], u['ig_page_id'], u['pipeline_id']))
        return jsonify({"success": True, "user": {"id": u['id'], "username": u['username'], "role": "admin" if u['username'] == 'admin' else "user", "pipeline_id": u['pipeline_id'], "meta_token": u['meta_token'], "ig_page_id": u['ig_page_id']}})
    return jsonify({"success": False, "error": "Usuário ou senha incorretos"})

@app.route('/api/auth/me')
def api_auth_me():
    if current_user.is_authenticated:
        return jsonify({"authenticated": True, "user": {"id": current_user.id, "username": current_user.username, "role": "admin" if current_user.username == 'admin' else "user", "pipeline_id": current_user.pipeline_id, "meta_token": current_user.meta_token, "ig_page_id": current_user.ig_page_id}})
...
# --- ROTAS LEGADAS (form-based) ---
@app.route('/login', methods=['POST'])
def login():
    conn=get_db(); cur=conn.cursor(); cur.execute("SELECT * FROM users WHERE username=?",(request.form['username'],)); u=cur.fetchone(); conn.close()
    if u and (check_password_hash(u['password'],request.form['password']) or u['password']==request.form['password']):
        login_user(User(u['id'],u['username'],u['password'],u['meta_token'],u['ig_page_id'],u['pipeline_id'])); return redirect(url_for('index'))
...
def adm_u(): c=get_db(); u=[dict(r) for r in c.execute("SELECT * FROM users").fetchall()]; c.close(); return jsonify({"users":u})
@app.route('/api/admin/users',methods=['POST'])
def adm_cu(): c=get_db(); c.execute("INSERT INTO users (username,password,pipeline_id) VALUES (?,?,1)",(request.json['username'],generate_password_hash(request.json['password']))); c.commit(); c.close(); return jsonify({"success":True})
@app.route('/api/admin/users/update',methods=['POST'])
def adm_uu(): d=request.json; c=get_db(); c.execute("UPDATE users SET meta_token=? WHERE id=?",(d.get('session_id'),d['id'])) if 'session_id' in d else None; c.execute("UPDATE users SET pipeline_id=? WHERE id=?",(d.get('pipeline_id'),d['id'])) if 'pipeline_id' in d else None; c.commit(); c.close(); return jsonify({"success":True})
@app.route('/api/admin/users/delete',methods=['POST'])
def adm_du(): c=get_db(); c.execute("DELETE FROM users WHERE id=?",(request.json['id'],)); c.commit(); c.close(); return jsonify({"success":True})
@app.route('/api/admin/pipelines')
def adm_p(): c=get_db(); ps=[dict(r) for r in c.execute("SELECT * FROM pipelines").fetchall()]; res=[{**p,"stages":[dict(s) for s in c.execute("SELECT * FROM stages WHERE pipeline_id=?",(p['id'],)).fetchall()]} for p in ps]; c.close(); return jsonify({"pipelines":res})
@app.route('/api/admin/pipelines',methods=['POST'])
def adm_cp(): c=get_db(); c.execute("INSERT INTO pipelines (name) VALUES (?)",(request.json['name'],)); c.commit(); c.close(); return jsonify({"success":True})
@app.route('/api/admin/pipelines/delete',methods=['POST'])
def adm_dp(): pid=request.json['id']; c=get_db(); c.execute("DELETE FROM leads WHERE pipeline_id=?",(pid,)); c.execute("DELETE FROM stages WHERE pipeline_id=?",(pid,)); c.execute("DELETE FROM pipelines WHERE id=?",(pid,)); c.commit(); c.close(); return jsonify({"success":True})
@app.route('/api/admin/stages',methods=['POST'])
def adm_cs(): d=request.json; c=get_db(); c.execute("INSERT INTO stages (name,color,pipeline_id,position) VALUES (?,?,?,99)",(d['name'],d['color'],d['pipeline_id'])); c.commit(); c.close(); return jsonify({"success":True})
@app.route('/api/admin/stages/delete',methods=['POST'])
def adm_ds(): c=get_db(); c.execute("DELETE FROM stages WHERE id=?",(request.json['id'],)); c.commit(); c.close(); return jsonify({"success":True})

# --- KANBAN / LEADS API (usado pelo frontend Next.js) ---
...
def api_kanban_data():
    ...
    leads = [dict(r) for r in c.execute("SELECT * FROM leads WHERE pipeline_id=?", (current_user.pipeline_id,)).fetchall()]
    c.close()
...
def api_update_stage():
    d = request.get_json()
    c = get_db()
    c.execute("UPDATE leads SET status=? WHERE id=?", (d['status'], d['id']))
    c.commit(); c.close()
    return jsonify({"success": True})
...
def api_delete_lead():
    d = request.get_json()
    c = get_db()
    c.execute("DELETE FROM leads WHERE id=?", (d['id'],))
    c.commit(); c.close()
    return jsonify({"success": True})
...
def api_create_lead():
    d = request.get_json()
    c = get_db()
    # Pega o primeiro estágio do pipeline como status inicial
    first_stage = c.execute("SELECT name FROM stages WHERE pipeline_id=? ORDER BY position ASC LIMIT 1", (current_user.pipeline_id,)).fetchone()
    status = first_stage['name'] if first_stage else 'NOVOS'
    c.execute("INSERT INTO leads (name, username, value, status, pipeline_id) VALUES (?,?,?,?,?)",
              (d['name'], d.get('username', ''), d.get('value', 0), status, current_user.pipeline_id))
    c.commit(); c.close()
    return jsonify({"success": True})

# --- DOCTORS SETTINGS API (NOVO) ---
...
@app.route('/')
def index():
    if not current_user.is_authenticated: return render_template_string(HTML_TEMPLATE)
    c=get_db()
    stages=[dict(r) for r in c.execute("SELECT * FROM stages WHERE pipeline_id=? ORDER BY position ASC",(current_user.pipeline_id,)).fetchall()]
    leads_map={s['name']:[] for s in stages}
    for r in c.execute("SELECT * FROM leads WHERE pipeline_id=?",(current_user.pipeline_id,)).fetchall():
        if r['status'] in leads_map: leads_map[r['status']].append(dict(r))
    
    # Médicos agora via DB
...
def clear_notifications():
    ...
    c = get_db(); pid = current_user.pipeline_id
    if lid: c.execute("UPDATE leads SET unread_count = 0 WHERE id = ?", (lid,))
    else: c.execute("UPDATE leads SET unread_count = 0 WHERE pipeline_id = ?", (pid,))
    c.commit(); c.close()
    return jsonify({"success": True})
...
def get_activities():
    ...
    res = []
    for r in rows:
        try: dt_obj = datetime.strptime(r['created_at'], '%Y-%m-%d %H:%M:%S')
        except: dt_obj = datetime.now()
        res.append({"description": r['description'], "user_name": r['username'], "user_initial": r['username'][0].upper(), "time_ago": format_relative_time(dt_obj)})
...
def get_appointments():
    ...
    c = get_db()
    # MODIFICAÇÃO: Selecionando todos os campos incluindo os novos
    appts = c.execute("SELECT * FROM appointments WHERE date_str = ? AND pipeline_id = ?", (date, current_user.pipeline_id)).fetchall()
    c.close()
    return jsonify({"appointments": [{"id": a['id'], "patient": a['patient_name'], "doctor": a['doctor_name'], "time": a['time_str'], "note": a['notes'], "color": a['color'], "procedure": a['procedure'], "duration": a['duration']} for a in appts]})
...
def create_appointment():
    ...
    if not color: color = '#3b82f6'
    duration = data.get('duration', 30)
    procedure = data.get('procedure', '')

    c.execute("INSERT INTO appointments (patient_name, doctor_name, date_str, time_str, notes, color, pipeline_id, procedure, duration) VALUES (?,?,?,?,?,?,?,?,?)", 
...
# NOVA ROTA PARA EDITAR CONSULTA
...
def update_appointment_api():
    ...
    c.execute("UPDATE appointments SET patient_name=?, notes=?, procedure=?, duration=?, color=? WHERE id=?", 
...
# NOVA ROTA PARA EXCLUIR CONSULTA
...
@login_required
def upd_stg(): 
    c=get_db(); lid = request.json['id']; new_status = request.json['status']
    lead = c.execute("SELECT name FROM leads WHERE id=?", (lid,)).fetchone()
    if lead: log_action(current_user.id, f"moveu <b>{lead['name']}</b> para a coluna <b>{new_status}</b>", current_user.pipeline_id)
    c.execute("UPDATE leads SET status=? WHERE id=?",(new_status,lid)); c.commit(); c.close(); return jsonify({"success":True})

@app.route('/delete_lead',methods=['POST'])
@login_required
def del_ld(): 
    c=get_db(); lid = request.json['id']
    lead = c.execute("SELECT name FROM leads WHERE id=?", (lid,)).fetchone()
    if lead: log_action(current_user.id, f"removeu o lead <b>{lead['name']}</b> do sistema", current_user.pipeline_id)
    c.execute("DELETE FROM leads WHERE id=?",(lid,)); c.commit(); c.close(); return jsonify({"success":True})
...
# ==============================================================================
# FUNÇÃO DE CONEXÃO CORRIGIDA (TOKEN PAGE + USER)
# ==============================================================================
...
def connect_instagram():
    ...
    if not token: return jsonify({"success": False, "error": "Token não fornecido."})
    
    try:
        # TENTATIVA 1: Tenta usar como Token de PÁGINA (Direto)
        # Isso resolve o erro (#100) e pega o ID do Instagram vinculado à página
        ...
        # Se funcionou direto (Token de Página)
        if 'instagram_business_account' in data_me:
            ig_page_id = data_me['instagram_business_account']['id']
        
        # TENTATIVA 2: Se não veio direto, tenta listar contas (Token de Usuário - Fallback)
        if not ig_page_id:
            # Só roda isso se a primeira falhar para evitar o erro de permissão
            if 'error' in data_me and data_me['error']['code'] == 100:
                 # Se der erro 100 aqui, é token de usuário tentando acessar endpoint errado, vamos tentar accounts
                 pass
            
            resp_accounts = requests.get(f"{GRAPH_URL}/me/accounts", params={'access_token': token})
            data_accounts = resp_accounts.json()
            
            # Se a lista de contas der erro, retorna o erro original
...
@login_required
def c_lead():
    d=request.json; c=get_db(); pid=current_user.pipeline_id; exist=c.execute("SELECT id FROM leads WHERE username=? AND pipeline_id=?",(d['username'],pid)).fetchone()
    if exist: 
        c.execute("UPDATE leads SET status=?,last_msg=?,profile_pic=?,last_interaction=? WHERE id=?",(d['target_stage'],d['last_msg'],d['profile_pic'],d['time_ago'],exist['id']))
        log_action(current_user.id, f"atualizou dados de <b>{d['name']}</b>", pid)
    else: 
        c.execute("INSERT INTO leads (name,username,last_msg,profile_pic,status,last_interaction,pipeline_id) VALUES (?,?,?,?,?,?,?)",(d['name'],d['username'],d['last_msg'],d['profile_pic'],d['target_stage'],d['time_ago'],pid))
...
@login_required
def chat_th():
    if not current_user.meta_token: return jsonify({"success":False,"error":"Instagram não conectado"})
    try:
        url = f"{GRAPH_URL}/{current_user.ig_page_id}/conversations"
        # CORREÇÃO: Removido 'platform': 'instagram'
        ...
        conversations = data.get('data', [])
        c=get_db()
        leads_q=c.execute("SELECT l.username,l.status,s.color FROM leads l JOIN stages s ON l.status=s.name AND l.pipeline_id=s.pipeline_id WHERE l.pipeline_id=?",(current_user.pipeline_id,)).fetchall(); c.close()
        ...
        for t in conversations:
            ...
            res.append({"thread_id": t['id'], "user": {"name": u.get('name', 'User'), "username": u.get('username', 'user'), "pic": ""}, "last_msg": last_msg[:40], "lead_info": l_map.get(u.get('username', ''))})
...
def chat_ms():
    ...
    try:
        ...
        for m in data.get('data', []):
            ...
            msgs.append({"text": m.get('message', '[Mídia]'), "is_sent_by_me": is_me})
